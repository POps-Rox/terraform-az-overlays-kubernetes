locals {
  ...
  cluster_name      = coalesce(var.custom_cluster_name, data.popsrox_utils_resource_name.aks.result)
  aks_identity_name = coalesce(var.aks_user_assigned_identity_custom_name, data.popsrox_utils_resource_name.aks_identity.result)
}
